...
with tab7:
    ...
    if 'symbols' not in st.session_state:
        ...
        # Caching biểu đồ: chỉ vẽ lại khi dữ liệu hoặc lựa chọn thay đổi
        @st.cache_data(show_spinner=False)
        def compute_cagr(df, col):
            df_sorted = df.sort_values('period')
            start_year = df_sorted['period'].iloc[0]
            start_val = df_sorted[col].iloc[0]
            years = df_sorted['period']
            cagr_values = []
            for y, val in zip(years, df_sorted[col]):
                period_diff = int(start_year) - int(y)  # theo yêu cầu: start_year - y
                if period_diff == 0:
                    cagr_values.append(None)
                else:
                    cagr_val = (val / start_val)**(1/period_diff) - 1
                    cagr_values.append(cagr_val * 100)
            return years, cagr_values

        # Giảm số lượng năm được vẽ nếu số năm quá nhiều (ví dụ chỉ lấy 10 năm mới nhất)
        def filter_years(df, selected_years, max_years=10):
            if selected_years:
                df_filtered = df[df['period'].isin(selected_years)]
            else:
                df_filtered = df.copy()
            if len(df_filtered) > max_years:
                df_filtered = df_filtered.sort_values('period', ascending=False).head(max_years)
        ...
        for symbol in symbols:
            ...
            with st.expander("Bảng cân đối kế toán (Hàng năm)"):
                ...
                if not balance_data.empty and 'period' in balance_data.columns:
                    ...
                    numeric_cols = [col for col in balance_data.select_dtypes(include=['float64', 'int64']).columns if col != 'period']
                    if numeric_cols:
                        selected_cols = st.multiselect(f"Chọn chỉ số (Bảng cân đối {symbol}):", options=numeric_cols, default=[])
                        available_years = sorted(balance_data['period'].unique())
                        selected_years = st.multiselect(f"Chọn năm (Bảng cân đối {symbol}):", options=available_years, default=[])
                        df_filtered = filter_years(balance_data, selected_years)
                        if selected_cols:
                            for col in selected_cols:
                                ...
                                with tab1:
                                    ...
                                    fig_bar.update_layout(title=f"{col} - {symbol}", xaxis_title="Period", yaxis_title="Giá trị (Tỷ đồng)", template="plotly_white", height=300)
                                    st.plotly_chart(fig_bar, use_container_width=True, config=config)
                                with tab2:
                                    if df_filtered.shape[0] >= 2:
                                        years, cagr_values = compute_cagr(df_filtered, col)
                                        fig_cagr = go.Figure(go.Scatter(
                                            x=years,
                                            y=cagr_values,
                                            mode='lines+markers',
                                            marker_color='red',
                                        ...
                                        fig_cagr.update_layout(title=f"CAGR của {col} - {symbol}", xaxis_title="Period", yaxis_title="CAGR (%)", template="plotly_white", height=300)
                                        st.plotly_chart(fig_cagr, use_container_width=True, config=config)
                                    else:
                                        st.info("Không đủ dữ liệu để tính CAGR.")
                else:
                    st.warning(f"Không có dữ liệu hoặc cột 'period' cho bảng cân đối của {symbol}")
            ...
            with st.expander("Báo cáo lãi lỗ (Hàng năm)"):
                ...
                if not income_data.empty and 'period' in income_data.columns:
                    ...
                    numeric_cols = [col for col in income_data.select_dtypes(include=['float64', 'int64']).columns if col != 'period']
                    if numeric_cols:
                        selected_cols = st.multiselect(f"Chọn chỉ số (Lãi lỗ {symbol}):", options=numeric_cols, default=[])
                        available_years = sorted(income_data['period'].unique())
                        selected_years = st.multiselect(f"Chọn năm (Lãi lỗ {symbol}):", options=available_years, default=[])
                        df_filtered = filter_years(income_data, selected_years)
                        if selected_cols:
                            for col in selected_cols:
                                tab1, tab2 = st.tabs(["Biểu đồ cột", "Biểu đồ CAGR"])
                                with tab1:
            ...
            with st.expander("Báo cáo lưu chuyển tiền tệ (Hàng năm)"):
                ...
                if not cash_flow_data.empty and 'period' in cash_flow_data.columns:
                    ...
                    numeric_cols = [col for col in cash_flow_data.select_dtypes(include=['float64', 'int64']).columns if col != 'period']
                    if numeric_cols:
                        selected_cols = st.multiselect(f"Chọn chỉ số (Lưu chuyển {symbol}):", options=numeric_cols, default=[])
                        available_years = sorted(cash_flow_data['period'].unique())
                        selected_years = st.multiselect(f"Chọn năm (Lưu chuyển {symbol}):", options=available_years, default=[])
                        df_filtered = filter_years(cash_flow_data, selected_years)
                        if selected_cols:
                            for col in selected_cols:
                                tab1, tab2 = st.tabs(["Biểu đồ cột", "Biểu đồ CAGR"])
                                with tab1:
                    ...
                    st.warning(f"Không có dữ liệu hoặc cột 'period' cho báo cáo lưu chuyển của {symbol}")
            ...
            with st.expander("Chỉ số tài chính (Hàng năm)"):
                ...
                if not ratios_data.empty and 'period' in ratios_data.columns:
                    ...
                    numeric_cols = [col for col in ratios_data.select_dtypes(include=['float64', 'int64']).columns if col != 'period']
                    if numeric_cols:
                        selected_cols = st.multiselect(f"Chọn chỉ số (Chỉ số {symbol}):", options=numeric_cols, default=[])
                        available_years = sorted(ratios_data['period'].unique())
                        selected_years = st.multiselect(f"Chọn năm (Chỉ số {symbol}):", options=available_years, default=[])
                        df_filtered = filter_years(ratios_data, selected_years)
                        if selected_cols:
                            for col in selected_cols:
                                tab1, tab2 = st.tabs(["Biểu đồ cột", "Biểu đồ CAGR"])
                                with tab1:
                                    ...
                                    fig_bar.update_layout(title=f"{col} - {symbol}", xaxis_title="Period", yaxis_title="Giá trị", template="plotly_white", height=300)
                                    st.plotly_chart(fig_bar, use_container_width=True, config=config)
                                with tab2:
                                    if df_filtered.shape[0] >= 2:
                                        years, cagr_values = compute_cagr(df_filtered, col)
